...
<template>
  ...
  <ion-popover v-model:is-open="isOpen" trigger="filter-popover" @willDismiss="emits('update')">
    <ion-content class="ion-padding-horizontal">
      <h4>Category</h4>
      <ion-chip v-for="(name, key) in ExerciseCategory" :key="key" :color="category.includes(key as ExerciseCategory) ? 'primary' : undefined" @click="toggleCategory(key as ExerciseCategory)">{{ name }}</ion-chip>
      <h4>Bodyparts</h4>
      <ion-chip v-for="(name, key) in ExerciseBodyPart" :key="key" :color="bodyPart.includes(key as ExerciseBodyPart) ? 'primary' : undefined" @click="toggleBodyPart(key as ExerciseBodyPart)">{{ name }}</ion-chip>
    </ion-content>
  </ion-popover>
</template>
